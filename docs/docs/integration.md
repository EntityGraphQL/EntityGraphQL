...
`MapGraphQL()` follows the [GraphQL over HTTP spec](https://github.com/graphql/graphql-over-http/blob/main/spec/GraphQLOverHTTP.md) (adopted in v6). GraphQL errors are returned in the response body - always read `errors` there. Status codes follow the spec: with `application/graphql-response+json` (the default) a request error returns `400` and a partial success `294`; with legacy `application/json` every well-formed request returns `200`.
:::
