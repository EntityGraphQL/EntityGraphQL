...
The GraphQL spec does not define how a server should implement this functionality. EntityGraphQL implements the [graphql-ws](https://github.com/enisdenjo/graphql-ws/blob/main/PROTOCOL.md) protocol. This uses web sockets to deliver messages between the client & server. This is well supported by libraries like [Apollo](https://www.apollographql.com/docs/react/data/subscriptions/).
...
Let's look at a simple chat event. You can see the full code in the [subscription example](https://github.com/EntityGraphQL/EntityGraphQL/tree/main/src/examples/subscriptions) on GitHub.
...
Following the [`graphql-ws` protocol](https://github.com/enisdenjo/graphql-ws/blob/main/PROTOCOL.md), once a client has sent the `connection_init` message and received the `connection_ack` message back it can send a `subscribe` message with a unique ID and the payload is the GraphQL subscription operation:
...
The [`graphql-ws`](https://github.com/enisdenjo/graphql-ws/blob/main/PROTOCOL.md#single-result-operation) protocol also supports executing GraphQL `query` and `mutation` operations over the connection. This is supported by EntityGraphQL and follows the specification. However typically clients will use HTTPS for `query` and `mutation` operations and web sockets for `subscription` operations to aid with scale.

## Other Implementations

The above GraphQL subscription implementation is based on [`graphql-ws`](https://github.com/enisdenjo/graphql-ws/blob/main/PROTOCOL.md) protocol which uses web sockets. It is provided by the [EntityGraphQL.AspNet](https://www.nuget.org/packages/EntityGraphQL.AspNet) package. The base [EntityGraphQL](https://www.nuget.org/packages/EntityGraphQL) package does not know about web sockets, there is potential to implement other transport layers or protocols on top of that.
...
// In your subscription or query
public string MySubscription(MyInput input) {
...
```csharp
public string MySubscription(Guid? id, string? name, IArgumentsTracker argsTracker) {
