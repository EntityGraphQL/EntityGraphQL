...
- New `[GraphQLFromContext]` on a `[GraphQLField]` method parameter binds it to a member of the field's context instead of exposing it as a GraphQL argument. This lets a `static` method with a body - one that loops, branches or `await`s, where an `Expression<Func<>>` cannot be used - be a service field on a type other than the root query type: the declared members (navigation properties included) are selected in the database-bound pass and the call is rewritten against that result, the same as `.Resolve()` dependency extraction, so nothing else on the entity is selected. Every dependency must be declared: a parameter taking the whole entity is rejected at schema build, as it would select the entity itself and a body reading a navigation property would silently see an empty collection. Instance methods on the entity remain unsupported - the entity is the call target and does not survive that pass - and now fail with a message naming the alternatives instead of an `ArgumentException` about a `Dynamic_` type. See [Declaring what a method reads from the context](https://entitygraphql.github.io/schema-creation/other-data-sources#declaring-what-a-method-reads-from-the-context).
