...
- The async result processing (run when a query contains async fields) no longer reflects over values whose type cannot contain an async result. Previously it read every property of every object in the result — which could trigger EF lazy-loading of navigation properties the query never selected — and rebuilt typed collections unnecessarily.
...
- **API surface** (breaking) — `LinqRuntimeTypeBuilder` and `ExpressionReplacer` are now `internal`; `ExpressionUtil.ListToSingleMethods` is no longer public (it was a mutable static). `GraphQLHelper.InjectServices` gained optional parameters and accepts a null service provider.
