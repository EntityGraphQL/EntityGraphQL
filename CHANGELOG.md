...
- Fixed a `ResolveBulk` field selected on data that the first (no-service) pass never produces failing the whole root field with `Field '<name>' - Error occurred` (`ArgumentException: Static property requires null instance...`, or `KeyNotFoundException` for the bulk data key). Cases below another service field's selection and with `ExecuteServiceFieldsSeparately = false` fall back to their per-item resolver (one service call per item) instead of failing. Root service-resolved lists/objects now participate in the two-pass flow and bulk-load (see 6.1.3). Repro tests from [#538](https://github.com/EntityGraphQL/EntityGraphQL/pull/538).
