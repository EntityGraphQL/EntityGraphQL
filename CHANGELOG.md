...
- Fixed a `ResolveBulk` field selected on data that the first (no-service) pass never produces failing the whole root field with `Field '<name>' - Error occurred` (`ArgumentException: Static property requires null instance...`, or `KeyNotFoundException` for the bulk data key). The clearest case is a root field resolved from services - `schema.Query().AddField("comments", ...).Resolve<IMyService, MyDbContext>(...)` selecting `{ isReadByCurrentUser }` - where the first pass produces no expression at all, so no bulk load is registered while the second pass still compiled the lookup; the same applied below another service field's selection and with `ExecuteServiceFieldsSeparately = false`. These fields now fall back to their per-item resolver (one service call per item) instead of failing. Bulk loading them needs the service-resolved parent executed before the keys are collected - a further execution pass, not implemented.
