...
- Fixed `ResolveBulk` fields failing when reached through a nested single-object navigation (e.g. `tasks { project { createdBy } }`) or a mutation result selection (e.g. `mutation { updateTask { project { createdBy } } }`) - single-object contexts are now wrapped for the bulk key batching, object navigations off a list context project per element, and the duplicate mutation result-selection path node is skipped by reference. Thanks [@alex-birch](https://github.com/alex-birch) [#526](https://github.com/EntityGraphQL/EntityGraphQL/pull/526)
