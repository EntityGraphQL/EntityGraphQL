...
- **Security (DoS)** — the GraphQL document parser now enforces a hard recursion-depth backstop (nested selection sets and nested argument/input values) during parsing, returning a `DocumentError` before a maliciously deep document can exhaust the stack. This runs independently of, and before, the configurable `ExecutionOptions.MaxQueryDepth`.
