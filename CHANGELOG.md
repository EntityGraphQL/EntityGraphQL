...
- A resolver can now be told what the engine will read off the objects it returns, so it can fetch only that instead of everything - the point of batching with `ResolveBulk` when the data comes from another database or service. Take an `IFieldSelection` parameter and the engine supplies it, the way it supplies `CancellationToken` and `QueryRequestContext`; new two-service `ResolveBulk`/`ResolveBulkAsync` overloads let a loader take it alongside its own service. It is not the caller's selection set: a selected field that is itself resolved from a service is replaced by the member its resolver reads, nested objects come through as paths (`Address.City`), `@skip`/`@include` are applied, fragments are expanded, aliases collapse, `__typename` is excluded, and one load is told the union of every place the field is selected. See [Fetching only the fields that will be used](https://entitygraphql.github.io/schema-creation/other-data-sources#fetching-only-the-fields-that-will-be-used).
- **Roslyn analyzers now ship in the package** - no extra install. They catch schema-building mistakes in the editor and at build time rather than at start-up or in production. Ten rules to start: `EGQL001` (Info) a `.Resolve<TService>()` field on an entity type with no `.ResolveBulk(...)` - the N+1 trap; `EGQL002` (Warning) an async field using a `DbContext`, which resolves concurrently on lists (`MaxQueryConcurrency` defaults to 100) and is not thread-safe; `EGQL003` (Warning) `UseFilter`/`UseSort`/`UseOffsetPaging`/`UseConnectionPaging`/`UseAggregate` on a field that does not return a collection; `EGQL004` (Warning, **with a code fix**) a synchronous `Resolve()` whose expression returns a `Task`; `EGQL005` (Warning) a name that is not a valid GraphQL name; `EGQL006` (Warning) a `[GraphQLSubscription]` method not returning an observable; `EGQL007` (Warning) a `[GraphQLOneOf]` input type with a non-nullable field; `EGQL008` (Warning) the same field added to a type twice; `EGQL009` (Info) a blocking `ExecuteRequest` inside an async method; `EGQL010` (Info) a `Resolve`/`ResolveBulk` that blocks on a `Task` with `.Result`/`.Wait()`/`GetAwaiter().GetResult()`. Most mirror exceptions already thrown when the schema is built, so they cannot break a build that was previously working. Every rule's severity can be changed (or turned off) per project in `.editorconfig` - see [Analyzers](https://entitygraphql.github.io/analyzers).
...
- Fixed a `ResolveBulk` field selected on data that the first (no-service) pass never produces failing the whole root field with `Field '<name>' - Error occurred` (`ArgumentException: Static property requires null instance...`, or `KeyNotFoundException` for the bulk data key). The clearest case is a root field resolved from services - `schema.Query().AddField("comments", ...).Resolve<IMyService, MyDbContext>(...)` selecting `{ isReadByCurrentUser }` - where the first pass produces no expression at all, so no bulk load is registered while the second pass still compiled the lookup; the same applied below another service field's selection and with `ExecuteServiceFieldsSeparately = false`. These fields now fall back to their per-item resolver (one service call per item) instead of failing. Bulk loading them needs the service-resolved parent executed before the keys are collected - a further execution pass, not implemented. Repro tests from [#538](https://github.com/EntityGraphQL/EntityGraphQL/pull/538).
- Fixed a `ResolveBulk`/`ResolveBulkAsync` resolver whose values are lists - e.g. `ResolveBulk<MyService, string, IEnumerable<Item>>(...)` returning `IDictionary<string, IEnumerable<Item>>` - failing with `Specified cast is not valid` (`InvalidCastException`) when the field was resolved. A list field's dotnet return type is promoted to `IQueryable<T>` and the bulk data lookup was built against `Dictionary<TKey, IQueryable<Item>>`, which the loaded `Dictionary<TKey, IEnumerable<Item>>` cannot be cast to (a dictionary's generic arguments are invariant). The lookup now takes its dictionary type from the resolver's own signature and casts to `IDictionary<,>`, so any dictionary implementation and value type the resolver declares works. Reported in [#534](https://github.com/EntityGraphQL/EntityGraphQL/issues/534) - thanks [@soilidokay](https://github.com/soilidokay).
- `ResolveBulk()`/`ResolveBulkAsync()` on a field with no resolver now throws `EntityGraphQLSchemaException` naming the field and the fix, instead of a `NullReferenceException` from inside the schema-building call. A bulk resolver builds on a resolver rather than replacing it - the per-item resolver runs whenever the bulk load cannot (e.g. the field is selected below another service field), so a bulk-only field could not resolve at all. Also from [#534](https://github.com/EntityGraphQL/EntityGraphQL/issues/534).
- Fixed an async field (`ResolveAsync`, or a `ResolveBulkAsync` field falling back to its per-item resolver) below a **root list resolved from services** leaving an unawaited `Task<T>` in the response instead of its value - typically surfacing as a serialized empty object or `{"result": ...}`-shaped noise for that field. Nested async results were resolved only when services executed in a single pass or on the second pass of a two-pass execution; a root field resolved from services has no first pass to run, so its one execution is the final one while `ExecuteServiceFieldsSeparately` is still on and it matched neither case. Keyed off the final execution now. The same field below a context/`DbSet` root list was always awaited.
- Fixed the same `ResolveBulk` field selected at more than one level of a self-referencing type - `comments { isReadByCurrentUser replies { isReadByCurrentUser } }` - failing with `KeyNotFoundException` for a key that was never loaded. The loaded data is keyed by the bulk resolver name, and each level was loaded separately, so the last level's data replaced the previous level's. Every level's keys now go into one load call.
...
- A missing required argument on a field taking an arguments class is now reported by its schema name instead of its dotnet property name - `missing required argument 'from'`, not `'From'`. `ArgType` already used the schema name, so the two validation paths disagreed and a query missing one argument could produce both spellings. Reading the capitalised name naturally suggests the schema stopped applying `SchemaFieldNamer` to arguments, which sends you looking in the wrong place entirely.
- `Variable or value used for argument 'x' does not match argument type 'Y' on field 'z'` now carries the reason the value could not be built (e.g. `Object must implement IConvertible.`) when the schema is in development mode or the exception is allowed via `AllowedExceptions`/`AllowedExceptionAttribute`. Both types in that message are GraphQL types and are frequently the same on each side, so the message could describe a conversion failing between two dotnet types while naming neither - the underlying exception was caught and discarded. Messages outside development are unchanged. `ISchemaProvider.IsAllowedException(Exception)` is new (it was already the private gate behind `AllowedExceptionMessage`) - custom `ISchemaProvider` implementations need to add it.

# 6.1.1

## Fixes

- Fixed `ResolveBulk` fields selected through a fragment spread or inline fragment failing with `KeyNotFoundException` (`The given key 'bulk_Type.field' was not present in the dictionary`) anywhere below a list-to-single selection - e.g. a mutation returning `ctx.Things.First(...)` selected as `thing { child { ...ChildFields } }`, or `thing(id: 1) { ...ThingFields }`. Apollo/Relay clients hoist selections into named fragments, so this showed up as soon as a bulk field was used from a real frontend. A fragment spread is parsed with the parent of the node it is spread on, so the "is this field selected directly on a list-to-single node?" check read the wrong node: bulk loading was skipped for the field while the second pass still compiled the lookup, leaving it reading data that was never loaded. Both the registration and the lookup now ask the compile context which node is compiling its selection set, so they always agree, and the per-compile copy of a list-to-single node keeps its to-single marker instead of looking like a plain list selection. A bulk field selected via a fragment directly on a list-to-single field now uses the per-item resolver, matching how it already behaved when selected directly. Repro and regression tests from [#536](https://github.com/EntityGraphQL/EntityGraphQL/pull/536) - thanks [@alex-birch](https://github.com/alex-birch).
...
- Dropped the `netstandard2.1` target. EntityGraphQL now targets `net8.0`, `net9.0` and `net10.0`. This also removes the `System.Text.Json`, `Microsoft.CSharp` and `System.ComponentModel.Annotations` package dependencies that only the netstandard target required.
- **Security (fail closed)** — authorization now denies by default when a requirement is present but cannot be satisfied. A bare `[GraphQLAuthorize]` / `[Authorize]` (no roles or policy) now correctly requires an authenticated user instead of being a silent no-op, and `IsAuthorized` requires `Identity.IsAuthenticated`. In `EntityGraphQL.AspNet`, a policy-protected field is now denied (rather than allowed through) when no `IAuthorizationService` is registered or the user is `null`. Fields/types with no authorization attribute are unaffected. This is a behavior change: previously-open access that relied on any of these gaps will now be blocked.
- **Introspection now respects authorization** — introspection queries (`__schema` / `__type`) only return the types and fields the requesting user is authorized to access, so anonymous users can no longer enumerate role/policy-protected type, field and argument names. `schema.ToGraphQLSchemaString()` is unchanged and always outputs the full schema (for SDL files / codegen). Note introspection output can now differ per user, which may affect schema-caching tooling running as an unprivileged user.
- **`ExecutionOptions.MaxQueryConcurrency` now defaults to 100** (previously unlimited). An async field resolved for a list runs per item, so an unbounded default turned a large result set into an unbounded number of concurrent operations (socket/thread-pool exhaustion, hammering rate-limited APIs). Set `MaxQueryConcurrency = null` to restore unlimited. Note this does not make non-thread-safe services (e.g. a scoped `DbContext`) safe in async list fields — use `ServiceConcurrencyLimits` or `maxConcurrency: 1` for those. Also fixed the concurrency wrapper for field shapes it previously failed on (value-type service parameters such as `CancellationToken`, rebound per-element parameters, nested list results — errors like `unbound variable` or `No coercion operator`), and `IAsyncEnumerable` fields are no longer wrapped (the stream's enumeration, not its creation, is the async work).
- API surface — `LinqRuntimeTypeBuilder` and `ExpressionReplacer` are now `internal`; `ExpressionUtil.ListToSingleMethods` is no longer public (it was a mutable static). `GraphQLHelper.InjectServices` gained optional parameters and accepts a null service provider. `ConcurrencyLimitFieldExtension.ExecuteWithConcurrencyLimitAsync` is now generic (`<TResult>`) so the wrapped expression keeps its static type.
- New GraphQL spec validations may reject previously accepted (invalid) documents — duplicate non-repeatable directives, conflicting field selection merges, `@skip`/`@include` on a subscription root field and deprecating a required argument are now errors. See the spec compliance entry under Changes.

## Changes

- New `schema.Query().AddFieldsFrom<T>()` (and any object type) plus the `schema.AddQueryFieldsFrom<T>()` shortcut - group related field definitions into classes using `[GraphQLField]` marked methods instead of many `AddField()` calls. Method parameters of the declaring type bind to the field context (the query context for root fields - this also applies to existing `[GraphQLField]` methods on the context type, which previously resolved the context from the service provider), other complex types resolve as services and remaining parameters become GraphQL arguments. Methods returning `IQueryable<T>` compose with EF - filters and selections translate to SQL.
- Updated dependencies: `Humanizer.Core` 2.14.1 -> 3.0.10 (used for singularizing names when building schemas - inflection rule changes could alter a generated name in rare cases), `Parlot` 1.4.3 -> 1.5.7, `System.Runtime.Caching` and `Microsoft.Extensions.Logging.Abstractions` 9.0.9 -> 10.0.9.
...
- **September 2025 GraphQL spec compliance** (document descriptions, full Unicode and OneOf runtime behavior shipped in earlier betas):
  ...
  - `__Directive.isRepeatable` added to introspection (all built-in directives are non-repeatable). Custom `IDirectiveProcessor` implementations can override the new `IsRepeatable` property (defaults to `false`).
  - Using a non-repeatable directive more than once at a single location (e.g. `@skip(...) @skip(...)`) is now a validation error.
  - `@skip`/`@include` on the root field of a subscription operation is now a validation error per the spec.
  - Arguments and input fields can now be deprecated (`[Obsolete]` on an argument property or `field.Deprecate(...)` on an input field). They are hidden from introspection by default with new `includeDeprecated` arguments on `__Field.args`, `__Directive.args` and `__Type.inputFields`, `__InputValue` gains `isDeprecated`/`deprecationReason`, and the SDL output annotates them with `@deprecated`. Deprecating a required argument is a schema error per the spec.
  - `Implements()` now validates field covariance — a field on an implementing type must be the same type or a sub-type of the interface's field (only fields present at the time of the call are checked, so the add-fields-after-implements pattern is unaffected).
  - Field selection merging is now validated: selections with the same response name must be the same field with identical arguments (`{ x: id x: name }` or `{ project(id: 1) project(id: 2) }` are errors — use different aliases), and across mutually exclusive fragments the response shapes must be compatible (`... on Cat { value: lives }` with `... on Dog { value: name }` is an error).
- `__type(name: ...)` now returns `null` for an unknown type name per the GraphQL spec, instead of an execution error.
- Async result processing is now targeted and compiled instead of a full-result reflection walk: only branches of the result whose types can hold an async value are visited (non-async branches are returned untouched). On a 10,000-row result with one async field and non-async nested collections this cut request time from ~140ms to ~63ms in benchmarks (the no-async baseline being ~19ms). This also stops the walk reading every property of every result object, which could trigger EF lazy-loading of navigation properties the query never selected.
- Subscription events are no longer executed synchronously on the publisher's thread. Each subscription processes its events on an ordered background queue - `observer.OnNext` returns immediately instead of blocking while the (potentially async/service-backed) result selection executes for every subscriber. Event order per subscription is preserved, including the terminal error/complete messages.
- **Policy evaluation is now truly async** — `PolicyOrRoleBasedAuthorization` pre-evaluates all policies used in the schema once at the start of each request (properly awaited) instead of blocking with sync-over-async `AuthorizeAsync` calls per field/type check during compilation. New optional `IGqlAuthorizationService.PrepareForRequestAsync` hook (has a default implementation — custom implementations are unaffected).
- **WebSocket hardening** — new `GraphQLWebSocketOptions` (optional parameter on `UseGraphQLWebSockets`): incoming messages larger than `MaxMessageSizeBytes` (default 1 MB) close the connection with `1009 MessageTooBig` instead of buffering unbounded; malformed JSON closes with `4400` instead of throwing out of the receive loop; and clients that never send `connection_init` are closed with `4408` after `ConnectionInitTimeout` (default 10s) per the graphql-ws protocol.
- **Security (DoS)** — the GraphQL document parser now enforces a hard recursion-depth backstop of 128 (nested selection sets and nested argument/input values) during parsing, returning a `DocumentError` before a maliciously deep document can exhaust the stack. This runs independently of, and before, the configurable `ExecutionOptions.MaxQueryDepth`.
- Hardening: the dynamic (projection) type cache now uses a full, collision-free key (field names, field types and parent type) instead of a 32-bit hash, removing an unlikely-but-possible hash collision handing a query a cached type with the wrong fields.

## Bug Fixes

- Fixed a compiled-delegate cache key collision when `CacheCompiledDelegates` is enabled and an operation has multiple root fields - the second root field was served the first field's compiled delegate, returning the wrong field's data. The cache key now includes the root field name.
- Fixed async argument validators (`IArgumentValidator` via `AddValidator<T>()`) being fire-and-forgotten — a validator doing real async work before adding its errors raced the request and its errors could be silently lost. Validators are now stored as async delegates and genuinely awaited for mutation and subscription arguments. Note validators on query field arguments still run during (synchronous) query compilation — avoid I/O in those. Breaking: `IField.Validators` is now `IReadOnlyCollection<Func<ArgumentValidatorContext, Task>>`.
- Fixed per-request state being written onto the cached query document (`EnableQueryCache`, on by default) during execution. Concurrent requests of the same query text share the parsed document; the dynamic types built for an interface/union selection during a request's first (database) pass were stored on the shared AST node and read in its second (services) pass, so an interleaved request whose `@include`/`@skip` variables expanded a different selection could poison another request's second pass — failing it with `No coercion operator is defined between types ...` or silently dropping fragment fields. That state now lives on the per-request compile context, and mutations returning lists no longer write their per-request parameters onto the shared selection node.
- Fixed `SchemaBuilder.FromObject` registering an abstract base class as an interface but not linking its implementing types to it (unless `AutoCreateInterfaceTypes` was on). The half-configured schema made inline fragments on the interface silently mis-project — `animals { ... on Cat { lives } }` returned the whole `Cat` object for `lives` instead of the value. `FromObject` now links every object type to any of its base types/interfaces that are registered in the schema as interfaces, regardless of the order the types were discovered in.
- Fixed `@include(if: $variable)` / `@skip(if: $variable)` on an **inline fragment** failing with `Variable or value used for argument 'if' does not match argument type 'Boolean!'`. The parser executed inline-fragment directives at parse time — before variable values exist (literal values worked). Directives on inline fragments are now processed at execution time like field directives.
- Async result processing fixes: a `Dictionary<,>` value in a result with async fields now stays a dictionary (previously it was turned into a list of key/value pairs, changing the serialized shape); user types that happen to live in an `EntityGraphQL.*` namespace are no longer treated as the engine's internal projection types and rebuilt; a non-generic `ValueTask` value is awaited instead of being walked as an object.
- Mutation and subscription methods can now declare a `CancellationToken` parameter and receive the executing request's token (previously this failed with a service-not-found/argument error, despite query field resolvers supporting it). The parameter is not exposed as a GraphQL argument.
- Fixed `UseFilter()`/`UseSort()` being silently ignored on a nested field whose own resolver is service-backed (e.g. `schema.Type<Project>().AddField(...).Resolve<Service>(...).UseFilter()`). Such a collection only exists in the services execution pass, where both extensions assumed the filtering/sorting had already happened in the first (database) pass — the query returned unfiltered/unsorted data with no error. Root-level service-backed fields were unaffected.
- Fixed three issues in the async field concurrency limiting (`maxConcurrency` / `MaxQueryConcurrency` / `ServiceConcurrencyLimits`): the per-request `CancellationToken` and limiter registry were baked into the expression as constants, so with `CacheCompiledDelegates` on, request 1's token lived inside the cached delegate and, once cancelled, failed every later request of that query (they are now passed as per-request execution parameters); a cancelled semaphore acquire leaked the permits already acquired (acquisition now happens inside the try/finally); and the wrapped operation lambda was compiled per list element (now compiled once per expression and cached).
- Fixed the (opt-in) `CacheCompiledDelegates` cache key hashing variable values with `ToString()`. Collection values (`List<>`, arrays, `Dictionary<,>`) stringify to their type name — identical for different contents — so two requests differing only in such a variable (e.g. a `sort` input) could share one cache entry and the second request would be served the first request's compiled plan (wrong ORDER BY field/direction). Values are now hashed by content, and a request whose variable values cannot be represented by value skips delegate caching for that request instead of risking a wrong-plan hit.
- Fixed bulk resolvers (`ResolveBulk`) leaking across sibling root fields. In a query with multiple root fields, a bulk resolver registered while executing one root field was re-run during each following root field's execution — re-calling the service (extra bulk loads) and, when the sibling's data had a different shape, failing that field with `Could not find field egql__... on type ...`.
- Fixed an operator-precedence bug in filter expression compilation that mis-detected string conversions: comparing a `DateTime`/`DateTimeOffset`/`TimeSpan`/`DateOnly`/`TimeOnly` field to a nullable value of the same type failed to compile, and a `Guid` vs `Guid?` comparison was built as `Guid.Parse(x.ToString())` (in-memory only, not translatable to SQL). These now use simple nullable promotion.
...
- Rewrote WebSocket subscription send path (`GraphQLWebSocketServer`) to use a `Channel`-backed drain task. All outbound frames are now serialised through a single writer, eliminating the concurrent `WebSocket.SendAsync` violation that could corrupt the frame stream when multiple subscriptions fired simultaneously. The `IObserver<T>` callbacks (`OnNext`/`OnError`/`OnCompleted`) now enqueue to the channel rather than blocking on `GetAwaiter().GetResult()`, freeing the observable/Redis thread immediately. `subscriptions` dictionary replaced with `ConcurrentDictionary` for thread-safety.
