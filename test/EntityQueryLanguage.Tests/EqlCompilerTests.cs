...
namespace EntityQueryLanguage.Tests {
	...
	public class EqlCompilerTests {
		...
		public void CompilesNumberConstant() {
			var exp = new EqlCompiler().Compile("3");
			Assert.Equal(3, exp.Execute());
		}

		[Fact]
		public void CompilesNegitiveNumberConstant() {
			var exp = new EqlCompiler().Compile("-43");
			Assert.Equal(-43, exp.Execute());
		}
		
		[Fact]
		public void CompilesNumberDecimalConstant() {
			var exp = new EqlCompiler().Compile("23.3");
			Assert.Equal(23.3m, exp.Execute());
		}
		
		[Fact]
		public void CompilesStringConstant() {
			var exp = new EqlCompiler().Compile("'Hello there_987-%#&	;;s'");
			Assert.Equal("Hello there_987-%#&	;;s", exp.Execute());
		}
		
		[Fact]
		public void CompilesIdentityCall() {
			var exp = new EqlCompiler().Compile("hello", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal("returned value", exp.Execute(new TestSchema()));
		}
		[Fact]
		public void FailsIdentityNotThere() {
			var ex = Assert.Throws<EqlCompilerException>(() => new EqlCompiler().Compile("wrongField", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider()));
		...
		public void CompilesIdentityCallFullPath() {
			var exp = new EqlCompiler().Compile("someRelation.field1", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(2, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesIdentityCallFullPathDeep() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(99, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesBinaryExpressionEquals() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id = 99", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
		...
		public void CompilesBinaryExpressionEqualsAndAdd() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id = (99 - 32)", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
		...
		public void FailsIfThenElseInlineNoBrackets() {
			...
			var ex = Assert.Throws<EqlCompilerException>(() => new EqlCompiler().Compile("someRelation.relation.id = 99 ? 'wooh' : 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider()));
		...
		public void CompilesIfThenElseInlineTrueBrackets() {
			...
			var exp = new EqlCompiler().Compile("(someRelation.relation.id = 99) ? 100 : 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
		...
		public void CompilesIfThenElseInlineFalseBrackets() {
			// tells it how to read it
			var exp = new EqlCompiler().Compile("(someRelation.relation.id = 98) ? 100 : 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesIfThenElseTrue() {
			var exp = new EqlCompiler().Compile("if someRelation.relation.id = 99 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(100, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesIfThenElseFalse() {
			var exp = new EqlCompiler().Compile("if someRelation.relation.id = 33 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesBinaryWithIntAndUint() {
			var exp = new EqlCompiler().Compile("if someRelation.UnisgnedInt = 33 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesBinaryWithNullableAndNonNullable() {
			var exp = new EqlCompiler().Compile("if someRelation.NullableInt = 8 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)), new DefaultMethodProvider());
			Assert.Equal(100, exp.Execute(new TestSchema()));
		}
