...
namespace EntityQueryLanguage.Tests {
	...
	public class EqlCompilerTests {
		...
		public void CompilesIdentityCall() {
			var exp = new EqlCompiler().Compile("hello", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal("returned value", exp.Execute(new TestSchema()));
		}
		[Fact]
		public void FailsIdentityNotThere() {
			var ex = Assert.Throws<EqlCompilerException>(() => new EqlCompiler().Compile("wrongField", new ObjectSchemaProvider(typeof(TestSchema))));
		...
		public void CompilesIdentityCallFullPath() {
			var exp = new EqlCompiler().Compile("someRelation.field1", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(2, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesIdentityCallFullPathDeep() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(99, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesBinaryExpressionEquals() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id = 99", new ObjectSchemaProvider(typeof(TestSchema)));
		...
		public void CompilesBinaryExpressionEqualsAndAdd() {
			var exp = new EqlCompiler().Compile("someRelation.relation.id = (99 - 32)", new ObjectSchemaProvider(typeof(TestSchema)));
		...
		public void FailsIfThenElseInlineNoBrackets() {
			...
			var ex = Assert.Throws<EqlCompilerException>(() => new EqlCompiler().Compile("someRelation.relation.id = 99 ? 'wooh' : 66", new ObjectSchemaProvider(typeof(TestSchema))));
		...
		public void CompilesIfThenElseInlineTrueBrackets() {
			...
			var exp = new EqlCompiler().Compile("(someRelation.relation.id = 99) ? 100 : 66", new ObjectSchemaProvider(typeof(TestSchema)));
		...
		public void CompilesIfThenElseInlineFalseBrackets() {
			// tells it how to read it
			var exp = new EqlCompiler().Compile("(someRelation.relation.id = 98) ? 100 : 66", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		
		[Fact]
		public void CompilesIfThenElseTrue() {
			var exp = new EqlCompiler().Compile("if someRelation.relation.id = 99 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(100, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesIfThenElseFalse() {
			var exp = new EqlCompiler().Compile("if someRelation.relation.id = 33 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesBinaryWithIntAndUint() {
			var exp = new EqlCompiler().Compile("if someRelation.UnisgnedInt = 33 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(66, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CompilesBinaryWithNullableAndNonNullable() {
			var exp = new EqlCompiler().Compile("if someRelation.NullableInt = 8 then 100 else 66", new ObjectSchemaProvider(typeof(TestSchema)));
			Assert.Equal(100, exp.Execute(new TestSchema()));
		}
		[Fact]
		public void CanUseCompiledExpressionInWhereMethod() {
			var exp = new EqlCompiler().Compile("name = 'Bob", new ObjectSchemaProvider(typeof(TestEntity)));
