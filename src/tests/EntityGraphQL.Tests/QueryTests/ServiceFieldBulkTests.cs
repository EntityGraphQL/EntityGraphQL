...
public class ServiceFieldBulkTests
{
    ...
    /// Queryable.WhereWhen against the materialised list. From #527.
    /// </summary>
    [Fact]
    public void TestScalarBulkOnWhereWhenRootList_WithSiblingRootField()
    {
        ...
        // sibling root service field exercises BulkResolvers.Clear() between root fields
    ...
    /// <summary>
    /// A custom directive that rebuilds the root list node (like an EF AsSplitQuery directive) without
    /// copying IsRootField. The engine preserves position-derived state on rebuilt nodes, so second-pass
    /// service execution still treats the node as root instead of re-applying Queryable.WhereWhen against
    /// the materialised list. From #527.
    /// </summary>
    [Fact]
...
/// <summary>Bulk service for the WhereWhen root list regression tests (#527).</summary>
...
/// <summary>Sibling root service field stand-in (#527).</summary>
public class FavoriteProjectsService
{
    public IEnumerable<int> GetFavoriteIds() => [1, 2];
}

/// <summary>Stand-in for EF Core RelationalQueryableExtensions.AsSplitQuery (#527).</summary>
...
/// <summary>
/// A directive that wraps the list expression and rebuilds the list node. With
/// <paramref name="preserveIsRootField"/> false it reproduces a rebuilt node that does not copy
/// IsRootField - the engine must preserve that state itself (#527).
/// </summary>
