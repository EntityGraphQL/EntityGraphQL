...
public class ArgumentTests
{
    ...
    public void ThrowsOnMissingRequiredArguments()
    {
        ...
        schema
            ...
            .AddField(
                "user",
                new { id = ArgumentHelper.Required<int>(), h = ArgumentHelper.Required<string>() },
                (ctx, param) => ctx.Users.FirstOrDefault(u => u.Id == param.id),
                "Return a user by ID"
            );
        var gql = new QueryRequest
        {
    ...
    [Fact]
    public void ObjectArgViaConstructor()
    {
        var schema = SchemaBuilder.FromObject<TestDataContext>();
