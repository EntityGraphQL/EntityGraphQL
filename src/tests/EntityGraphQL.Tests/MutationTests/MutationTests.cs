...
public class MutationTests
{
    ...
    public void SupportsMutationArrayArg()
    {
        ...
        {
            Query =
                @"mutation AddPerson($names: [String]) {
  ...
  addPersonNamesAsync(names: $names) {
    ...
    {
        // This tests that when a mutation returns an Expression
        ...
        var vars1 = new QueryVariables { { "names", new[] { "Bill", "Frank" } } };
        var testSchema = new TestDataContext();
        ...
        results = schemaProvider.ExecuteRequestWithContext(
            new QueryRequest
            {
                Query = query,
    ...
    public void SupportsMutationVariablesAnonObject()
    {
        ...
        {
            Query =
                @"mutation AddPerson($names: InputObject) {
          addPersonInput(nameInput: $names) {
            id name lastName
          }
        }",
    ...
    public void SupportsSelectionWithServiceFieldInFragment()
    {
        ...
        {
            type.AddField("age", "Person's age").Resolve<AgeService>((p, service) => service.GetAge(p.Birthday));
        });
        schemaProvider.AddMutationsFrom<PeopleMutations>(new SchemaBuilderOptions() { AutoCreateInputTypes = true });
        // Add a argument field with a require parameter
        var gql = new QueryRequest
    ...
    public void TestNullableGuid()
    {
        ...
        {
            Query =
                @"mutation Mute($id: ID, $int: Int, $float: Float, $double: Float, $bool: Boolean, $enum: Gender) {
