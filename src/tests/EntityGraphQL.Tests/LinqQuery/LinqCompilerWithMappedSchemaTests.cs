...
namespace EntityGraphQL.LinqQuery.Tests
{
    ...
    public class LinqCompilerWithMappedSchemaTests
    {
        ...
        {
            var exp = EqlCompiler.Compile("people.where(guid = \"6492f5fe-0869-4279-88df-7f82f8e87a67\")", new TestObjectGraphSchema(), null);
        ...
        {
            var exp = EqlCompiler.Compile("people", new TestObjectGraphSchema(), null);
            dynamic result = exp.Execute(GetDataContext());
            Assert.Equal(1, Enumerable.Count(result));
        }
        [Fact]
        public void CompilesIdentityCallFullPath()
        {
            ...
            var exp = EqlCompiler.Compile("privateProjects.where(id = 8).count()", schema, null);
            Assert.Equal(0, exp.Execute(GetDataContext()));
            var exp2 = EqlCompiler.Compile("privateProjects.count()", schema, null);
        ...
        public void CompilesTypeBuiltFromObject()
        {
            ...
            var exp = EqlCompiler.Compile("defaultLocation.id = 10", new TestObjectGraphSchema(), null);
            Assert.True((bool)exp.Execute(GetDataContext()));
        }
        [Fact]
        public void CompilesIfThenElseInlineFalseBrackets()
        {
            var exp = EqlCompiler.Compile("(publicProjects.Count(id = 90) = 1) ? \"Yes\" : \"No\"", new TestObjectGraphSchema(), null);
            Assert.Equal("Yes", exp.Execute(GetDataContext()));
        }
        [Fact]
        public void CompilesIfThenElseTrue()
        {
            var exp = EqlCompiler.Compile("if publicProjects.Count() > 1 then \"Yes\" else \"No\"", new TestObjectGraphSchema(), null);
