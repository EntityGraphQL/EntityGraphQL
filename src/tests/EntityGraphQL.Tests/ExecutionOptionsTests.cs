...
public class ExecutionOptionsTests
{
    ...
    {
        var schema = SchemaBuilder.FromObject<TestDataContext>();
        schema.Query().AddField("projectsPaged", ctx => ctx.Projects, "Paged projects")
            .UseConnectionPaging();
        schema.Query().AddField("projectItems", ctx => ctx.Projects, "Paged projects")
            .UseOffsetPaging();
        var data = new TestDataContext();
        FillProjectData(data);
    ...
    [Fact]
    public void TestBeforeExpressionBuildScalar() => TestBeforeExpressionBuildExpression(
        "{ totalPeople }",
    ...
    [Fact]
    public void TestBeforeExpressionBuildOffsetPaging() => TestBeforeExpressionBuildExpression(
        "query MyOp { projectItems { items { name } } }",
        AssertExpression.Conditional(
            AssertExpression.Any(),
            AssertExpression.Any(),
            AssertExpression.MemberInit([AssertExpression.MemberBinding("items",
                AssertExpression.Call(null, nameof(EnumerableExtensions.ToListWithNullCheck),
                    AssertExpression.Call(null, "Select", AssertExpression.Call(null, "TagWith", AssertExpression.Any(), AssertExpression.AnyOfType(typeof(Action))), AssertExpression.Any()),
                    AssertExpression.Any()
        ))])),
        "projectItems", "MyOp");

    [Fact]
    public void TestBeforeExpressionBuildConnectionPaging() => TestBeforeExpressionBuildExpression(
        ...
        AssertExpression.Conditional(
            ...
            AssertExpression.MemberInit([AssertExpression.MemberBinding("edges",
                AssertExpression.Call(null, nameof(EnumerableExtensions.ToListWithNullCheck),
                    AssertExpression.Call(null, "Select",
                        AssertExpression.Call(null, "Select",
                            AssertExpression.Call(null, "Select",
    ...
    public void TestBeforeExpressionBuildInvalid()
    {
        ...
        var gql = new QueryRequest
        {
            Query = @"{ projects { name tasks { name id } } }",
        };
