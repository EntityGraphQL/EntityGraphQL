...
namespace EntityGraphQL.Compiler.EntityQuery.Tests
{
    public class DefaultMethodProviderTests
    {
        ...
        {
            var ex = Assert.Throws<EntityGraphQLCompilerException>(() => EntityQueryCompiler.Compile("people.where()", SchemaBuilder.FromObject<TestSchema>(), null, new DefaultMethodProvider()));
            Assert.Equal("Method 'where' expects 1 argument(s) but 0 were supplied", ex.Message);
        }
        [Fact]
        public void FailsWhereWrongParameterType()
        {
            var ex = Assert.Throws<EntityGraphQLCompilerException>(() => EntityQueryCompiler.Compile("people.where(name)", SchemaBuilder.FromObject<TestSchema>(), null, new DefaultMethodProvider()));
        ...
        {
            var exp = EntityQueryCompiler.Compile("people.first()", SchemaBuilder.FromObject<TestSchema>(), null, new DefaultMethodProvider());
            var result = exp.Execute(new TestSchema()) as Person;
        ...
        {
            var exp = EntityQueryCompiler.Compile("people.take(1)", SchemaBuilder.FromObject<TestSchema>(), null, new DefaultMethodProvider());
            var result = exp.Execute(new TestSchema()) as IEnumerable<Person>;
            Assert.Single(result);
        ...
        {
            var exp = EntityQueryCompiler.Compile("people.Skip(1)", SchemaBuilder.FromObject<TestSchema>(), null, new DefaultMethodProvider());
            var result = exp.Execute(new TestSchema()) as IEnumerable<Person>;
