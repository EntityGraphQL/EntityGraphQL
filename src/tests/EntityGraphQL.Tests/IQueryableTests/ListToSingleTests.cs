    ...
    public class ListToSingleTests
    {
        ...
        public void TestListToSingle()
        {
            ...
            data.Movies.AddRange(
                new Movie { Id = 10, Name = "A New Hope", Actors = [new() { Id = 1, Name = "Alec Guinness" }, new() { Id = 2, Name = "Mark Hamill" }] });
            data.SaveChanges();
        ...
        public void TestListToSingleWithFind()
        {
            ...
            data.Actors.AddRange(
                    new List<Actor> { new() { Id = 1, Name = "Alec Guinness" }, new() { Id = 2, Name = "Mark Hamill" } });
            data.SaveChanges();
            var result = schema.ExecuteRequest(gql, serviceProvider, null);
            Assert.Null(result.Errors);
