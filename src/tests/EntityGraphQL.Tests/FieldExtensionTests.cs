...
public class FieldExtensionTests
{
    ...
    public void TestConnectionPagingWithOthers()
    {
        ...
        schema.Query().ReplaceField("people", ctx => ctx.People, "Return list of people with paging metadata").UseFilter().UseSort().UseConnectionPaging();
    ...
    public void TestOffsetPagingWithOthers()
    {
        ...
        schema.Query().ReplaceField("people", ctx => ctx.People, "Return list of people with paging metadata").UseFilter().UseSort().UseOffsetPaging();
        var gql = new QueryRequest
        {
            Query =
                @"{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void TestOffsetPagingWithOthersAndServices(bool separateServices)
    {
        var schema = SchemaBuilder.FromObject<TestDataContext>();
        var data = new TestDataContext();
        FillData(data);
    ...
    public void TestConnectionPagingWithOthersAndServicesNonRoot()
    {
        ...
        schema.Type<Project>().ReplaceField("tasks", ctx => ctx.Tasks.OrderBy(p => p.Id), "Return list of task with paging metadata").UseFilter().UseSort().UseConnectionPaging(defaultPageSize: 2);
        schema.AddType<ProjectConfig>("ProjectConfig").AddAllFields();
        schema.Type<Task>().AddField("config", "Task config").Resolve<ConfigService>((t, srv) => srv.Get(t.Id));
        var gql = new QueryRequest
        {
            Query =
                @"{
