...
directiveCall:
	ws* '@' name = identity (
		ws* '(' ws* directiveArgs = gqlargs ws* ')'
	)? ws*;
aliasType: name = identity ws* ':' ws*;
field:
	alias = aliasType? fieldDef = identity argsCall = gqlCall? ws* directive = directiveCall? ws*
...
operationName:
	operation = identity ('(' (operationArgs = gqlTypeDefs)? ')')?;
dataQuery:
	wsc* (queryKeyword | (queryKeyword ws* operationName))? ws* objectSelection;
mutationQuery:
	wsc* mutationKeyword ws* operationName ws* objectSelection;
subscriptionQuery:
	wsc* subscriptionKeyword ws* operationName ws* objectSelection;
gqlFragment:
	wsc* 'fragment' ws+ fragmentName = identity ws+ 'on' ws+ fragmentType = identity ws* fields =
		objectSelection;
...
graphQL:
	gqlFragment* (dataQuery | mutationQuery | subscriptionQuery) (
		dataQuery
		| mutationQuery
		| subscriptionQuery
		| gqlFragment
...
expression:
	'if ' (' ' | '\t')* test = expression (' ' | '\t')* 'then ' (
		' '
		| '\t'
	)* ifTrue = expression (' ' | '\t')* 'else ' (' ' | '\t')* ifFalse = expression				# ifThenElse
